...
export const ChatInterface: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-transparent overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-5 scrollbar-hide data-stream-bg">
            ...
            >
              <div className={`max-w-[85%] flex items-start gap-3 ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`w-7 h-7 rounded-full border flex items-center justify-center shrink-0 transition-all duration-500 ${
                  ...
                  {msg.sender === 'user' ? <Shield className="w-3.5 h-3.5 text-secondary" /> : <Cpu className="w-3.5 h-3.5 text-primary" />}
                </div>

                {/* Message Bubble with Glassmorphism */}
                <div className={`p-3 rounded-xl relative overflow-hidden group ${
      ...
      <div className="p-4 bg-black/60 border-t border-white/10 relative overflow-hidden">
        ...
        <div className="relative group">
          ...
          <input
            ...
            className="w-full bg-white/5 border border-white/10 focus:border-primary/50 focus:bg-primary/5 pl-10 pr-10 py-2.5 text-xs font-mono text-white placeholder:text-white/20 outline-none transition-all rounded"
          />
          <button
            onClick={handleSend}
            disabled={!input.trim()}
            className="absolute right-1.5 top-1.5 h-7 w-7 bg-primary/20 hover:bg-primary text-primary hover:text-white flex items-center justify-center transition-all duration-300 rounded disabled:opacity-20"
          >
